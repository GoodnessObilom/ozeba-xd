...
cmd({
  pattern: "cinfo",
  alias: ["newsletter", "id", "cid", "channelinfo"],
  use: ".cinfo <link>",
  react: "📡",
  ...
  try {
    ...
    if (metadata.preview && typeof metadata.preview === 'string' && metadata.preview.startsWith('/')) {
      try {
        ...
        console.error("Failed to send preview image, sending text only:", e);
